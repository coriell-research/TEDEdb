...
getOutlierContrasts <- function(sefiles, cores) {
  ...
  keep <- c("BioProject", "BioSample", "group", "group_id", "salmon_version", "library_types",
            "avg_frag_length_mean", "avg_frag_length_sd", "avg_num_eq_classes", 
            "avg_num_processed", "avg_num_mapped", "avg_num_decoy_fragments", 
            "avg_num_dovetail_fragments", "avg_num_fragments_filtered_vm", 
            "avg_num_alignments_below_threshold_vm", "avg_percent_mapped")
  metadata <- metadata[, ..keep]
  metadata[, library_layout := data.table::fifelse(library_types %like% "^[IOM]", "PAIRED", "SINGLE")]
  ...
  metadata[, `:=`(is_outlier_avg_frag_length_mean  = coriell::outliers_by_mad(avg_frag_length_mean),                                
                  ...
                  is_outlier_avg_num_eq_classes  = coriell::outliers_by_mad(avg_num_eq_classes, direction="low"),                               
                  is_outlier_avg_num_processed  = coriell::outliers_by_mad(avg_num_processed, direction="low"),                                
                  is_outlier_avg_num_mapped  = coriell::outliers_by_mad(avg_num_mapped, direction="low"),                                      
                  is_outlier_avg_num_decoy_fragments  = coriell::outliers_by_mad(avg_num_decoy_fragments, direction="high"),                     
                  is_outlier_avg_num_dovetail_fragments  = coriell::outliers_by_mad(avg_num_dovetail_fragments, direction="high"),                  
                  is_outlier_avg_num_fragments_filtered_vm  = coriell::outliers_by_mad(avg_num_fragments_filtered_vm, direction="high"),             
                  is_outlier_avg_num_alignments_below_threshold_vm  = coriell::outliers_by_mad(avg_num_alignments_below_threshold_vm, direction="high"),
  ...
  has_outlier_avg_frag_length_mean <- metadata[is_outlier_avg_frag_length_mean == TRUE, unique(group_id)]
  has_outlier_avg_frag_length_sd <- metadata[is_outlier_avg_frag_length_sd == TRUE, unique(group_id)]
  has_outlier_avg_num_eq_classes <- metadata[is_outlier_avg_num_eq_classes == TRUE, unique(group_id)]
  has_outlier_avg_num_processed <- metadata[is_outlier_avg_num_processed == TRUE, unique(group_id)]
  has_outlier_avg_num_mapped <- metadata[is_outlier_avg_num_mapped == TRUE, unique(group_id)]
  has_outlier_avg_num_decoy_fragments <- metadata[is_outlier_avg_num_decoy_fragments == TRUE, unique(group_id)]
  has_outlier_avg_num_dovetail_fragments <- metadata[is_outlier_avg_num_dovetail_fragments == TRUE, unique(group_id)]
  has_outlier_avg_num_fragments_filtered_vm <- metadata[is_outlier_avg_num_fragments_filtered_vm == TRUE, unique(group_id)]
  has_outlier_avg_num_alignments_below_threshold_vm <- metadata[is_outlier_avg_num_alignments_below_threshold_vm == TRUE, unique(group_id)]
  has_outlier_avg_percent_mapped <- metadata[is_outlier_avg_percent_mapped == TRUE, unique(group_id)]
  ...
  con_outlier_avg_frag_length_mean <- cm_dt[outlier_avg_frag_length_mean == TRUE, unique(id)]
  con_outlier_avg_frag_length_sd <- cm_dt[outlier_avg_frag_length_sd == TRUE, unique(id)]
  con_outlier_avg_num_eq_classes <- cm_dt[outlier_avg_num_eq_classes == TRUE, unique(id)]
  con_outlier_avg_num_processed <- cm_dt[outlier_avg_num_processed == TRUE, unique(id)]
  con_outlier_avg_num_mapped <- cm_dt[outlier_avg_num_mapped == TRUE, unique(id)]
  con_outlier_avg_num_decoy_fragments <- cm_dt[outlier_avg_num_decoy_fragments == TRUE, unique(id)]
  con_outlier_avg_num_dovetail_fragments <- cm_dt[outlier_avg_num_dovetail_fragments == TRUE, unique(id)]
  con_outlier_avg_num_fragments_filtered_vm <- cm_dt[outlier_avg_num_fragments_filtered_vm == TRUE, unique(id)]
  con_outlier_avg_num_alignments_below_threshold_vm <- cm_dt[outlier_avg_num_alignments_below_threshold_vm == TRUE, unique(id)]
  con_outlier_avg_percent_mapped <- cm_dt[outlier_avg_percent_mapped == TRUE, unique(id)]

  # Return a list of these IDs for use in the main annotation script
  result <- list(outlier_avg_frag_length_mean = con_outlier_avg_frag_length_mean,
